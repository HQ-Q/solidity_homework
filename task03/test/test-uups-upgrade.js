...
describe("UUPS Upgrade NFTAuction", function () {
    it("should upgrade NFTAuction contract using UUPS", async function () {
        ...
        console.log("test-uups----nftAddress::", nftAddress);
        // mint 10个 NFT
        for (let i = 0; i < 10; i++) {
        ...
        console.log("test-uups----代理合约地址----::", nftAuctionProxy.address);
        const nftAuction = await ethers.getContractAt(
            "NFTAuction",
            nftAuctionProxy.address
        );
        // 先授权NFT给拍卖合约
        console.log("test-uups----正在授权NFT给拍卖合约...");
        await nft.approve(nftAuctionProxy.address, 0);
        console.log("test-uups----NFT授权完成");
        console.log("test-uups----创建拍卖...");
        ...
        console.log("test-uups----创建拍卖成功:", auction);
        const implAddress1 = await upgrades.erc1967.getImplementationAddress(
            nftAuctionProxy.address
        );

        // 直接在测试中对现有 proxy 执行升级（不要再次运行 fixture，会重新部署依赖并覆盖 proxy）
        const NFTAuctionV2 = await ethers.getContractFactory("NFTAuctionV2");
        console.log("test-uups----upgrading proxy in-test...");
        await upgrades.upgradeProxy(nftAuctionProxy.address, NFTAuctionV2);
        const implAddress2 = await upgrades.erc1967.getImplementationAddress(
            nftAuctionProxy.address
        );
        //4.验证拍卖数据未丢失
        const auction2 = await nftAuction.auctions(0);
        console.log("test-uups----升级后读取拍卖成功:", auction2);
        console.log("test-uups----实现合约1地址:", implAddress1);
        console.log("test-uups----实现合约2地址:", implAddress2);
